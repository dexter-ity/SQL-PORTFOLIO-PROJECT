...
CREATE TABLE regions ( region_id INTEGER, region_name VARCHAR(9)
);
...
CREATE TABLE customer_nodes ( customer_id INTEGER,
region_id INTEGER, node_id INTEGER, start_date DATE, end_date DATE
);
...
CREATE TABLE customer_transactions ( customer_id INTEGER,
txn_date DATE, txn_type VARCHAR(10), txn_amount INTEGER
);
...
FROM customer_nodes;
#question 2
SELECT re.region_name, COUNT(cn.node_id) AS nodes_per_region
...
FROM customer_nodes cn
JOIN region_table r ON cn.region_id = r.region_id
GROUP BY r.region_name;
#question 4
with daysdifference as 
(select customer_id,node_id, datediff(end_date, start_date) as diff
from customer_nodes where end_date not like '%9999%')
select node_id, round(avg(diff),2) as avg_diff
from daysdifference
group by node_id
order by node_id;*/
#question 5 
...
/*B
#question 1
select distinct(count(txn_type)), sum(txn_amount)
from customer_transactions;
#question 2
SELECT round(AVG(deposit_count),2) AS avg_dep_count, round(AVG(deposit_amount),2) AS avg_dep_amount
FROM (SELECT COUNT(*) AS deposit_count,SUM(txn_amount) AS deposit_amount
FROM customer_transactions
WHERE txn_type = 'deposit'
GROUP BY customer_id) AS deposit_summary;
#question 3
SELECT YEAR(txn_date) AS year,MONTH(txn_date) AS month,COUNT(DISTINCT customer_id) AS more_transactions_customers
FROM customer_transactions
WHERE txn_type IN ('deposit', 'purchase', 'withdrawal')
GROUP BY YEAR(txn_date), MONTH(txn_date), customer_id
HAVING COUNT(CASE WHEN txn_type = 'deposit' THEN 1 END) > 1 AND COUNT(CASE WHEN txn_type IN ('purchase', 'withdrawal') THEN 1 END) >= 1;
#question 4
WITH monthly_transactions AS (
    SELECT customer_id,DATE_FORMAT(txn_date, '%Y-%m') AS month,SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) AS monthly_balance
    FROM customer_transactions
    GROUP BY customer_id, DATE_FORMAT(txn_date, '%Y-%m')
),
cumulative_balance AS (
    SELECT customer_id,month,monthly_balance,SUM(monthly_balance) OVER (PARTITION BY customer_id ORDER BY month) AS closing_balance
...
WITH monthly_closing_balance AS (
    SELECT customer_id,DATE_FORMAT(txn_date, '%Y-%m') AS month,SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) AS closing_balance
    FROM customer_transactions
    GROUP BY customer_id, DATE_FORMAT(txn_date, '%Y-%m')
)
SELECT ROUND(SUM(CASE WHEN ((closing_balance - previous_closing_balance) / previous_closing_balance) * 100 > 5 THEN 1 ELSE 0 END) / COUNT(*) * 100, 2) AS percentage_increase
FROM 
    (
        SELECT customer_id,month,closing_balance,
